...
class yinma(Scene):
    def construct(self):
        ...
        dotb=always_redraw(lambda :Dot(RIGHT*bx.get_value()+UP*by.get_value(),color=BLUE))
        dotb2=always_redraw(lambda :Dot(RIGHT*bx.get_value()+DOWN*by.get_value(),color=GREEN))
        l=Line(LEFT*5,RIGHT*5)
        self.play(Create(l))
        self.play(Create(dota),Create(dotb),Create(dotp),Create(dotb2))
        ap=always_redraw(lambda :Line(dota.get_center(),dotp.get_center(),color=YELLOW))
        ...
        self.play(bx.animate.increment_value(-2),by.animate.increment_value(1),run_time=2)
        self.play(bx.animate.increment_value(1),by.animate.increment_value(-1),run_time=2)

...
class creates(Scene):
    def jumpin(self, dot2, x_goto=0, y_goto=0):
        ...
        def run(dot2):
            ...
            dot2.move_to(
                np.array([lis[place, 0]-last+x_goto, lis[place, 1], 0]))
...
# xxyy=point.centre
    def createangle3(self, x=2, y=1, xx=0, yy=0):
        ...
        self.angle = VMobject().set_points_as_corners(
            [point1, [0, 0, 0], point3])
    ...
    def drawarc3(self, line, radian=PI, time=1):
        ...
        self.play(Rotating(line, radians=radian, about_point=pointt1,
                  run_time=time, rate_func=smooth))
        self.remove(path)
...
class m1(creates):
    def construct(self):
        ...
        t1 = Text('二等分角的作法：')
        t2 = Tex('(1)在OA、OB上分别截取OD、OE，使OD=OE',
...
# 射线
        ...
        self.wait()
        self.play(FadeOut(line2, dot3, self.angle,
                  g, letter, aa, bb, c, reddot))
